<template>
  <main class="flex flex-col gap-y-10">
    ...
    <form v-on:submit.prevent="postExpenses" class="flex gap-40 justify-center items-center text-xl mr-10">
      <section class="flex flex-col gap-y-10">
        <aside>
          <label for="type">Payment Type: </label>
          <select v-model.lazy="expenseObj.type" name="type" class="w-80 h-10 p-1">
        ...
        <aside>
          ...
          <input v-model.lazy="expenseObj.date" type="date" name="date" id="date" placeholder="Date" class="border border-black w-80 h-10 p-1">
        </aside>
      </section>
      <section class="flex flex-col gap-y-10">
        <aside>
          <label for="name" class="ml-[1.1rem]">Name: </label>
          <input v-model.lazy="expenseObj.name" type="text" id="name" name="name" placeholder="What was the expense?" class="h-10 border w-80 p-1 border-black placeholder-gray-500">
        </aside>
        <aside>
          <label for="amount">Amount: </label>
          <input v-model.lazy="expenseObj.amount" type="text" name="amount" id="amount" placeholder="Amount" class="h-10 p-1 border w-80 border-black">
        </aside>
      </section>
    </form>
...
export default {
  ...
  data(){
      return {
         expensesApi: "http://localhost/expense_tracker/php/rest/api/V1/expense_api.php",
         expenses: [],
         expenseObj: {
           type: '',
           name: '',
